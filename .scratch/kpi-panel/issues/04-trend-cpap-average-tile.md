...
- [x] 用 01 的日期範圍函式取 W1 週一到今天（含）的 logs，只取 CPAP 時數為正數的日子。
- [x] 平均顯示一位小數，副標含「CPAP 平均」與「門檻 4」。4 引用常數。
- [x] 無資料顯示「—」，不出現 NaN。
- [x] 達標與未達標用不同顏色，沿用既有 tile 樣式，不引入新樣式系統。
- [x] smoke_test.js 新增：塞 W0 有填、W1 部分填部分空，renderTrend 後磚字串等於只取 W1 有填日的平均；全空時顯示 —。
